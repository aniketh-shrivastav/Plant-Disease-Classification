...
def parse_args():
    ap = argparse.ArgumentParser(description="Predict PlantVillage archive image using trained Kernel SVM")
    ap.add_argument("image_path", type=str, help="Path to image to predict")
    ap.add_argument("--model_dir", type=str, default=str(Path("outputs")/"archive_kernel_svm"))
    ap.add_argument("--kernel", type=str, default="rbf", choices=["linear", "poly", "rbf"])
    ap.add_argument("--gamma", type=float, default=0.05)
    ap.add_argument("--degree", type=int, default=3)
    ap.add_argument("--threshold", type=float, default=None, help="Optional override for decision threshold; if set, ignores saved threshold.json")
...
def main():
    ...
    # Use shared feature extractor from training module (HOG + HSV)
    feat = extract_features(Path(args.image_path))
    if pca is not None:
    ...
    # Allow CLI override of threshold for quick calibration/testing
    eff_thr = float(args.threshold) if args.threshold is not None else float(thr)
    pred = int(s >= eff_thr)
