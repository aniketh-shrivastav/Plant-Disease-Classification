...
def parse_args():
    ap = argparse.ArgumentParser(description="Train Kernel SVM for Apple (healthy vs diseased) using HOG+HSV")
    ap.add_argument("--data_dir", type=str, default=str(Path("apple")))
    ap.add_argument("--output_dir", type=str, default=str(Path("outputs")/"apple_kernel_svm"))
...
def main():
    ...
    svm, pca_art, scaler, metrics = train_and_eval_apple(
        X,
        y,
    ...
    print("Apple Kernel SVM training complete.")
