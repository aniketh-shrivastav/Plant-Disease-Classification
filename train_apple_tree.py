...
def parse_args():
    ap = argparse.ArgumentParser(description="Train scratch Decision Tree for Apple (healthy vs diseased) using HOG+HSV")
    ap.add_argument("--data_dir", type=str, default=str(Path("apple")))
    ap.add_argument("--output_dir", type=str, default=str(Path("outputs")/"apple_tree"))
...
def main():
    ...
    print(f"Loading Apple data from: {data_dir}")
    ...
    print("Apple Decision Tree training complete.")
