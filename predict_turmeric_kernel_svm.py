...
def load_artifacts(model_dir: Path):
    ...
    # Load kernel hyperparameters (optional) from metrics.json
    kernel = "rbf"
    gamma = 0.05
    degree = 3
...
def main():
    ap = argparse.ArgumentParser(description="Predict Turmeric disease with scratch Kernel SVM")
    ap.add_argument("--image", required=True, type=str)
    ap.add_argument("--model_dir", type=str, default=str(Path("outputs")/"turmeric_kernel_svm"))
    args = ap.parse_args()
    ...
    for xi in x:
        ...
        for a, y_i, x_sv in zip(alpha, y_sv, X_sv):
            if a > 0:
                s += a * y_i * kernel_eval(x_sv, xi, kernel, gamma, degree)
        s += b
        margins.append(s)
