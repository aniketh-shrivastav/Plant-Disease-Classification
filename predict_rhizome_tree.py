...
def predict_one_tree(x: np.ndarray, node):
    # node is either a class id (int) or a dict with feature, threshold, left, right
...
def main():
    ap = argparse.ArgumentParser(description="Predict rhizome disease with scratch Decision Tree")
    ap.add_argument("--image", required=True, type=str)
    ap.add_argument("--model_dir", type=str, default=str(Path("outputs")/"rhizome_tree"))
    args = ap.parse_args()
