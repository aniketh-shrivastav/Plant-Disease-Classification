...
def parse_args():
    ap = argparse.ArgumentParser(description="Train Kernel SVM for Dragon Fruit (healthy vs diseased) using HOG+HSV")
    ap.add_argument("--data_dir", type=str, default=str(Path("Dragon Fruit (Pitahaya)")/"Original Images"))
    ap.add_argument("--output_dir", type=str, default=str(Path("outputs")/"dragon_kernel_svm"))
...
def main():
    ...
    svm, pca_art, scaler, metrics = train_and_eval_dragon(
        X,
        y,
    ...
    print("Dragon Fruit Kernel SVM training complete.")
